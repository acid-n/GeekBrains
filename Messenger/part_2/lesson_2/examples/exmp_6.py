# Теперь посмотрим как создать метакласс для своих пользовательских классов
#
# Представьте, что мы устали от задания атрибутов в конструкторе __init__(self, *args, **kwargs)
# Как бы сделать так, чтобы мы могли задавать атрибуты непосредственно при создании экземпляра класса
...
# Создадим метакласс наследованием от type - метакласс будет переопределять данный оператор ()

class AttrOptim(type):
    def __call__(self, *args, **kwargs):
        # Вызов класса создает новый объект
        # Сначала создадим сам объект
        obj = type.__call__(self, *args)
        # и добавим ему переданные в вызове аргументы в качестве атрибута
        for name in kwargs:
